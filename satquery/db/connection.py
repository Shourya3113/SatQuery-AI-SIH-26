...
A single-file SQLite store — no Docker PostGIS, no FastAPI connection flakiness
during the build. Schema lives in ``satquery.db.models``; this module owns the
connection pool (one reusable connection per store) and the spatial queries.

Public contract (re-exported from ``satquery.db``)::

    save_raster_record(metadata: dict) -> raster_id
    get_raster_by_id(raster_id: str)   -> dict | None
    list_raster_records()              -> list[dict]
"""
...
DEFAULT_DB_PATH = Path(__file__).resolve().parent.parent.parent / "data" / "cache" / "satquery_cache.db"
...
def save_raster_record(metadata: dict) -> str:
    """Persist a raster metadata dict (e.g. ``load_raster(path).to_dict()`` or a
    ``RasterInspection.as_dict()``) and return the generated ``raster_id``."""
