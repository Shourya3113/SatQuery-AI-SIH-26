...
try:
    import numpy as np
    import rasterio
    from satquery.services import geospatial
except Exception:  # pragma: no cover - exercised on machines without the venv
    np = rasterio = None
    geospatial = None
...
# Affine used by the sample rasters (see scripts/make_sample_tiffs.py).
...
class TestMockContract(unittest.TestCase):
    ...
    def test_seeded_deterministic(self):
        a1, b1 = geospatial.generate_mock_tensors(32, seed=42)
        a2, b2 = geospatial.generate_mock_tensors(32, seed=42)
        self.assertTrue(np.array_equal(a1, a2))
        self.assertTrue(np.array_equal(b1, b2))
        a3, _ = geospatial.generate_mock_tensors(32, seed=43)
        self.assertFalse(np.array_equal(a1, a3))


class TestRasterIngestion(unittest.TestCase):
    """Deliverable 2: GeoTIFF/COG ingestion + (C, H, W) model slices."""

    def test_load_optical_metadata(self):
        r = geospatial.load_raster(str(OPT))
        self.assertTrue(r.ok and r.real)
...
class TestSARAndIndices(unittest.TestCase):
    ...
    def test_indices_formulas(self):
        ...
        self.assertEqual(float(geospatial.ndvi(0.0, 0.0)), 0.0)  # no NaN at zeros
    ...
    def test_raster_index_resolves_bands(self):
        ...
        # equals the raw-band computation on the same raster
        r = geospatial.load_raster(str(OPT))
        manual = geospatial.ndvi(r.arrays[3], r.arrays[2])  # B08 vs B04
...
class TestAffineProjector(unittest.TestCase):
    ...
    def test_block_projects_to_25_ha(self):
        # 50 x 50 pixels at 10 m -> 500 m x 500 m = 25.0000 ha
        ...
        # reprojected into the WGS84 / lon-lat neighborhood of the sample origin
    ...
    def test_two_regions_summed_and_filtered(self):
        m = np.zeros((128, 128), dtype="uint8")
        m[40:90, 50:100] = 1   # 25 ha
        m[10:20, 10:20] = 1    # 10x10 px -> 100 m x 100 m = 1 ha
        gj = geospatial.mask_to_geojson(m, transform=SAMPLE_TRANSFORM, crs="EPSG:32633")
        self.assertEqual(len(gj["features"]), 2)
        self.assertAlmostEqual(gj["satquery"]["total_area_ha"], 26.0, places=4)
        small = geospatial.mask_to_geojson(m, transform=SAMPLE_TRANSFORM, crs="EPSG:32633",
                                           min_area_ha=2.0)
    ...
    def test_half_pixel_pixel_to_world(self):
        # pixel (row=40, col=50) center -> 500000 + 10*(50+0.5) = 500505
        x, y = geospatial.pixel_to_world(SAMPLE_TRANSFORM, 40, 50)
        self.assertAlmostEqual(x, 500_505.0, places=6)
        self.assertAlmostEqual(y, 5_399_595.0, places=6)  # 5400000 - 10*(40+0.5)
        # inverse round-trips
    ...
    def test_geographic_crs_measured_via_utm(self):
        # single 1x1 pixel of ~0.0001 deg (~81 m^2) in a geographic raster
    ...
    def test_no_shapely_fallback(self):
        # Force the rasterio-only path: still valid GeoJSON, simplify skipped.
